...
if auth.has_membership('administrators'):
    response.menu.extend([(SPAN(T(' Edit'), _class='icon-cog'), False,
                           URL('plugin_blog', 'editing',
                               args=['articles']),
                           [(SPAN(T(' Articles'), _class='icon-home'), False,
                             URL('plugin_blog', 'editing',
                                 args=['articles'],
                                 vars={'orderby': 'created'}),
                             []),
                            (SPAN(T(' Tags'), _class='icon-tag'), False,
                             URL('plugin_blog', 'editing',
                                 args=['blog_tags'],
                                 vars={'orderby': 'tagname'}),
                             []),
                            (SPAN(T(' Documents'), _class='icon-file'), False,
                             URL('plugin_blog', 'editing',
                                 args=['docs'],
                                 vars={'orderby': 'label'}),
                             []),
                            (SPAN(T(' Database'), _class='icon-cog'), False,
                             URL('appadmin', 'index'),
                             []),
                            ]),
                          ])
